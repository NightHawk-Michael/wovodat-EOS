...
// Added this if statement on 4-FEb-2013. because station_id can be null so it will be problem below...
	if($station_id != ""){
		...
		// SELECT vd_id FROM cn WHERE cn_id=276 LIMIT 1
		...
		// No result, SELECT vd_id FROM jj_volnet WHERE jj_net_id=cn_id AND jj_net_flag=C
		$query_sql="SELECT vd_id FROM jj_volnet WHERE jj_net_id =".$network_id." AND jj_net_flag=".$network_type;
		$query_results=array();
		$query_error="";
		if (!db_sql($query_sql, $query_results, $query_error)) {
			// Database error
