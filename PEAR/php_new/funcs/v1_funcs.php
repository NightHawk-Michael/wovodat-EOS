...
function v1_set_ms(&$object, $obj_name, $obj_code, $obj_stime, $obj_etime, $target_name, $target_key, $checked_key, $parent_key, $parent_id, $global_var, &$error) {
	...
	foreach ($global_var as $gen_code) {
		if (!empty($gen_code)) {
			...
			// Search in DB
			if ($checked_key=="dn" || $checked_key=="gn" || $checked_key=="hn" || $checked_key=="fn" || $checked_key=="tn" || $checked_key=="mn") {     //Nang added on 6-Sep-2012
...
function v1_set_ms_data(&$object, $obj_name, $obj_code, $obj_stime, $obj_etime, $target_name, $table_name, $target_key, $checked_key, $parent_name, $parent_table_name, $parent_key, $parent_id, $global_var, &$error) {
	...
	foreach ($global_var as $gen_code) {
		if (!empty($gen_code)) {
			...
			// Search in DB
			if ($checked_key=="dn" || $checked_key=="gn" || $checked_key=="hn" || $checked_key=="fn" || $checked_key=="tn" || $checked_key=="mn") {   // $checked_key=="mn" Nang added on 6-Sep-2012
...
//Nang comment out on 6-Sep-2012 because of changed idea. It won't take pubDate value from xml bigger than starttime+2years. It's only possible to get value from within stime and stime+2yrs	
...
function v1_get_id_cn_stime($table, $code, $stime, $owners) {
	...
	switch ($table) {
		...
		case "tn":
			$type="Thermal";
			break;
		case "mn":          // Nang added on 15-may-2012
			$type="Meteo";
			break;			
	}
	...
	return $row['cn_id'];   //Nang added on 09-feb-2012   changed from "$row[$table.'_id']" to $row['cn_id']
...
function v1_get_id_cn($table, $code, $owners) {
	...
	switch ($table) {
		...
		case "tn":
			$type="Thermal";
			break;
		case "mn":        // Nang added on 14-May-2012
			$type="Meteo";
			break;			
	}	
	// Create SQL query
...
/******************************************************************************************************
By Nang
* Function to get station's start time and end time by checking station name "ss_code"
******************************************************************************************************/
function v1_get_code($gen_stations) {
	...
	$sql="select ss.ss_stime,ss.ss_etime from ss where ss.ss_code='{$gen_stations[0]}'";
...
Written By Nang
* Function to get the ID for a certain code in a certain table
* Returns All the values of respective tables
* Input:	- $table: the table name in the database
* 			- $code: the code to search
* 			- $owners: an array of owners

function v1_get_allfields($table, $code, $stime, $owners) {
	...
	// Create SQL query
	if($table =='dn' || $table == 'gn' || $table == 'hn' || $table == 'fn' || $table == 'tn' || $table == 'mn' ){
		$sql="SELECT * FROM cn WHERE cn_code='".mysql_real_escape_string($code)."' AND cn_type='".$type."' AND cn_stime='".mysql_real_escape_string($stime)."' AND (";
	}
	else{
		$sql="SELECT * FROM ".$table. " WHERE " .$table."_code='".mysql_real_escape_string($code)."' AND ".$table."_stime='".mysql_real_escape_string($stime)."' AND (";
	}
...
* Returns FALSE if an error occurred, TRUE otherwise
* Input:	- $insert_table: the name of the table in database
* 			- $field_name: an array of field names
* 			- $field_value: an array of field values
* Output:	- $error: an error message and its code

function v1_insert_history($backup_table, $bk_field_name, $bk_field_value, &$errors){
	...
	$field_name=count($bk_field_name);
	$field_value=count($bk_field_value);

	if ($field_name!=0 && $field_name!=$field_value) {
		$errors="Error in the parameters given";
		return FALSE;
	}
	...
	$sql="insert into ". $backup_table ."(".$bk_field_name[0];
	
	for($i=1;$i<$field_name;$i++){
		$sql.= ",". $bk_field_name[$i];
	}
	
	$sql.= ") values ('".$bk_field_value[0];
	
	for($i=1;$i<$field_value;$i++){
		$sql.= "','". $bk_field_value[$i];
	}
