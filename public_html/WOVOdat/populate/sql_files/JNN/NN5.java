...
class MYNNBP {
	...
	public MYNNBP Calculate_Output_and_Derivatives(){
		...
		Matrix Eack = new Matrix(Eacobi,Kn);
		Matrix weights = ((Jack.transpose().times(Jack)).plus(Matrix.identity(dim,dim).times(nu))).inverse().times(Jack.transpose().times(Eack));
...
public class NN5 {
	...
	Faji.Weights_Factors_and_Bias = "Weights_Factors_and_Bias_for_NO2.dat";
	Faji.SSE_criteria = 0.0001;
	Faji.Process_ANN_Training(true,true,1);	
	
	/*atrix A = new Matrix(new double[][]{{2,1},{0,-1},{0,2}});
	Matrix B = new Matrix(new double[][]{{1,2},{-1,2}});
	A.print(2,2);
	A.transpose().print(2,2);
	A.transpose().times(A).print(2,2);*/
