...
class MYNNBP {
	...
	public MYNNBP Normalize_Data(){
		...
		for (int k=0;k<Kn;k++){
			...
			Output[k]=2*(Output[k]-GBmin[In])/(GBmax[In]-GBmin[In])-1;
		}
		return this;
	}
	...
	public MYNNBP Calculate_Output_and_Derivatives(){
		...
		for (int k=0;k<Kn;k++){
			...
			for (int j=0;j<Hn;j++){
				...
				for (int i=0;i<In;i++){
					dE_dVij[i][j] = dE_dVij[i][j] + del * Wj[j] * (1.0-Math.pow(Hj[j],2));
				}
			}
		}
	...
	public MYNNBP Initiate_Training_Variables(String DataFile,int Hidden_neurons){
		...
		momentum = 0.00003;
		Learn_rate_Bias = 0.00000001;
		Learn_rate_Weight = 0.00002;
		return this;
	}
...
public class NN3 {
	...
	Faji.Weights_Factors_and_Bias = "Weights_Factors_and_Bias_for_NO2_HL.dat";
