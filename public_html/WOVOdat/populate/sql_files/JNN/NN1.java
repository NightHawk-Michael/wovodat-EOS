...
class MYNNBP {
	...
	public MYNNBP Calculate_Output_and_Derivatives(){
		...
		for (int k=0;k<Kn;k++){
			...
			dE_dB = dE_dB + del;
			dE_dAlp = dE_dAlp + del * EHjWj;
			for (int j=0;j<Hn;j++){
				dE_dWj[j] = dE_dWj[j] + del * Alp * Hj[j];
				dE_dCj[j] = dE_dCj[j] + del * Alp * Wj[j] * Hj[j]*(1.0-Hj[j]);
				dE_dBj[j] = dE_dBj[j] + del * Alp * Wj[j] * Hj[j]*(1.0-Hj[j]) * EIiVij[j];
				for (int i=0;i<In;i++){
					dE_dVij[i][j] = dE_dVij[i][j] + del * Alp * Wj[j] * Hj[j]*(1.0-Hj[j]) * Bet[j]*Ii[i];
				}
			}
		}
	...
	public MYNNBP Initiate_Training_Variables(String DataFile,int Hidden_neurons){
		...
		momentum = 0.00057;
		Learn_rate_Factor = 0.0;
		Learn_rate_Bias = 0.0;
		Learn_rate_Weight = 0.00099;
		return this;
	}
...
public class NN1 {
	...
	Faji.Weights_Factors_and_Bias = "Weights_Factors_and_Bias_for_NO2_100.2.txt";
	Faji.SSE_criteria = 0.0001;
	Faji.Process_ANN_Training(true,true,100);
