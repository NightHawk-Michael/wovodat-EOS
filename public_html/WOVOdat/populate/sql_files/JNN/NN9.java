...
class MYNNBP {
	...
	public MYNNBP Normalize_Data(){
		...
		for (int k=0;k<Kn;k++){
			for (int i=0;i<In;i++){
				Input[k][i]=(Input[k][i]-GBmin[i])/(GBmax[i]-GBmin[i]);
			}
	...
	public MYNNBP Randomized_Weights_Factors_Bias(){
		...
		for (int j=0;j<Hn;j++){
			Wj[j]=r.nextGaussian();
			Cj[j]=r.nextGaussian();
			Bet[j] = r.nextGaussian();
			for (int i=0;i<In;i++){
				Vij[i][j]=r.nextGaussian();  
			}
		}
		return this;
	}   
	...
	public double Estimate_Out(double[] Ii){
		...
		for (int j=0;j<Hn;j++){
			...
			EIiVij[j] = Hj[j];
			Hj[j]=1.0/(1.0+Math.exp(-(Bet[j]*Hj[j]+Cj[j])));
		}
		double out=0;
		for (int j=0;j<Hn;j++){
			out=out+Hj[j]*Wj[j];
		...
		EHjWj = out;
		out=1.0/(1.0+Math.exp(-(Alp*out+b)));
		return out;
	}
	...
	public MYNNBP Calculate_Output_and_Derivatives(){
		...
		for (int k=0;k<Kn;k++){			
			...
			jindex++;
			for (int j=0;j<Hn;j++){
				Jacobi[k][jindex] = del * Alp * Hj[j];
				dE_dWj[j] = dE_dWj[j] + Jacobi[k][jindex];				
				jindex++;
				Jacobi[k][jindex] = del * Alp * Wj[j] * Hj[j]*(1.0-Hj[j]);
				dE_dCj[j] = dE_dCj[j] + Jacobi[k][jindex];				
				jindex++;
				Jacobi[k][jindex] = del * Alp * Wj[j] * Hj[j]*(1.0-Hj[j]) * EIiVij[j];
				dE_dBj[j] = dE_dBj[j] + Jacobi[k][jindex];				
				jindex++;
				for (int i=0;i<In;i++){
					Jacobi[k][jindex] = del * Alp * Wj[j] * Hj[j]*(1.0-Hj[j]) * Bet[j]*Ii[i];
	...
	public MYNNBP Test_Validation_Set(String Test_set){
		...
		for (int k=0;k<KnV;k++){
			for (int i=0;i<In;i++){
				InputV[k][i]=(InputV[k][i]-GBmin[i])/(GBmax[i]-GBmin[i]);   
			}
			OutputV[k]=(OutputV[k]-GBmin[In])/(GBmax[In]-GBmin[In]);
		}
	...
	public MYNNBP Predict_Properties(){
		...
		for (int k=0;k<Kn;k++){
			double Out = Estimate_Out(Input[k])*(GBmax[In]-GBmin[In])+GBmin[In];
	...
	public String Print_Equation(){
		...
		for (int j=0;j<Hn;j++){
			...
			Hj="1.0/(1.0+Math.exp(-("+Bet[j]+"*("+Hj+")"+Cj[j]+")))";
		}
		String out=Wj[0]+"*("+Hj+")";
		for (int j=1;j<Hn;j++){
			out=out+"+"+Wj[j]+"*("+Hj+")";
	...
	public void Process_ANN_Training(boolean Valid, boolean rand,int hidden){
		...
		double orig_learning = Learn_rate_Weight;
		while (this.SSE>SSE_criteria){
			...
			if (Valid==true){
				...
				if (SSE_old >= this.SSEV){
					...
					nu = nu/1.005;
					SSE_old = this.SSEV;	
					System.out.println("Best SSE ="+SSE_old);
					...
					if(piter<pitermax){				
						...
						if(miter>1400 && miter<=1500){
							momentum = momentum + (Math.random()*2-1)/100;
							Learn_rate_Weight = Learn_rate_Weight + (Math.random()*2-1)/100;;
						}
						if(miter>1500){
							Randomized_Weights_Factors_Bias();
							store_weights();
							...
							Learn_rate_Weight = orig_learning;
...
public class NN9 {
	...
	for(int i=1;i<=20;i++){
		...
		Faji.Prediction_output = "3NO2_valid_check_"+i+".dat";
		Faji.SSE_criteria = 0.0001;
		Faji.pitermax = 100;
		Faji.momentum = 0.7;
		Faji.Learn_rate_Weight = 0.2;
		Faji.Weights_Factors_and_Bias = "3Weights_Factors_and_Bias_for_NO2_"+i+".dat";
		Faji.Process_ANN_Training(true,false,i);	
		Faji.Process_ANN_Prediction();
		double endTime = System.nanoTime();
		double duration = (endTime - startTime)/6.0E+10;
		System.out.println("Nuerons ="+i+" SSE = "+Faji.SSE_from_prediction+" R^2 = "+Faji.Rsquared+" Execution = "+duration+" min");
	}
