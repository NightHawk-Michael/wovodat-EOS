...
class MYNNBP {
	...
	public MYNNBP Calculate_Output_and_Derivatives(){
		...
		for (int k=0;k<Kn;k++){
			...
			dE_dB = dE_dB + del;
			for (int j=0;j<Hn;j++){
				dE_dWj[j] = dE_dWj[j] + del * Hj[j];
				for (int i=0;i<In;i++){
					dE_dVij[i][j] = dE_dVij[i][j] + del * Wj[j] * Hj[j]*(1.0-Hj[j]);
				}
			}
		}
	...
	public MYNNBP Initiate_Training_Variables(String DataFile,int Hidden_neurons){
		...
		momentum = 0.003;
		Learn_rate_Bias = 0.000001;
		Learn_rate_Weight = 0.000004;
		return this;
	}
...
public class NN2 {
	...
	Faji.Weights_Factors_and_Bias = "Weights_Factors_and_Bias_for_NO2_SL.dat";
