		...
		function(){
			...
			for (var ih=0; ih<len_gwc;ih++){				
					$.post('helpers/process_gd_data.php',{
						...
						gd_time  			 : $('sampling_time input').eq(0).val()+" "+$('sampling_time input').eq(1).val(),
						gd_gtemp 			 : $('gas_temperature input').val(),
						gd_bp				 : $('gas_pressure input').val(),
						gd_flow				 : $('gas_emission_rate input').val(),
						gd_species			 : gas_with_conc[ih][5],
						gd_waterfree_flag	 : gas_with_conc[ih][2],
						gd_units			 : gas_with_conc[ih][4],
						gd_concentration	 : gas_with_conc[ih][0],
						gd_concentration_err : gas_with_conc[ih][1],
						gd_recalc			 : gas_with_conc[ih][3],	
						gd_envir			 : com.eq(0).val(),
						gd_submin			 : com.eq(1).val(),
						gd_ori				 : sel.eq(0).val(),
						gd_com				 : com.eq(2).val(),
						cc_id2 		  	     : sel.eq(1).find('option:selected').attr('my_id'),
						cc_id3 			     : sel.eq(2).find('option:selected').attr('my_id'),		
						gd_pubdate           : pub_date($('#pub_date').val()),
						cb_ids               : sel.eq(3).val().toString()
					},function(data){
						$('#data_happen').append(data);
						$('#data_happen').show();
						//setTimeout(function(){$('#data_happen').hide()}, 5000);
					});
			}
		}
	)
...
<body>
<h3 align='center' class='text-success'>INPUT FORM FOR DIRECTLY SAMPLED GAS</h3><br/><br/>
<div class="gd container">
		...
		<br/><br/><br/><br/>
		<space_holder class="col-sm-2 text_center">
		</space_holder>
		<gas_temperature class="col-sm-2 text_center">
		    <br/>
		    <label>Gas temperature(<sup>o</sup>C):</label><br/>
			<input type="text" placeholder="XXX.XX">
		</gas_temperature>
		<gas_pressure class="col-sm-2 text_center">
		    <label>Barometric pressure(mbar):</label><br/>
			<input type="text" placeholder="XXX.XX">
		</gas_pressure>
		<gas_emission_rate class="col-sm-2 text_center">
		    <label>Gas emission <br/>rate (t/d):</label><br/>
			<input type="text" placeholder="XXX.XX">
		</gas_emission_rate>
		<br/><br/><br/><br/><br/><br/>
		<h4 class='text-success'>Gas concentrations:</h4><br/>
		<gas_concentrations class="col-sm-12">
			<table class="table-hover table-condensed" id="gas_conc">   
				<tr><th>Gas <br/>species:</th><th>Gas <br/>concentration:</th><th>Concentration <br/>uncertainty:</th><th>Water free?</th><th>Recalculated</th><th>Reported Unit</th></tr>
				<tr>
				     <td>CO<sub>2</sub></td>
				     <td><input type="text" placeholder="XXX.XX"></td>
					 <td><input type="text" placeholder="XXX.XX"></td>
					 <td><input type="checkbox" value="Y">Yes &nbsp <input type="checkbox" value="N">No</td>
					 <td><input type="checkbox" value="R">Yes &nbsp <input type="checkbox" value="O">No</td >
					 <td><input type="text" placeholder="wt%; vol%, mole%, etc."></td>
				</tr>
				<tr>
				     <td>SO<sub>2</sub></td>
				     <td><input type="text" placeholder="XXX.XX"></td>
		...
		<comments class="col-sm-12" id="com">
			<label>Environmental factors</label>:&nbsp <input type="text" placeholder="Max255 characters"><br/>
			<label>Sublimate minerals</label>:&nbsp <input type="text" placeholder="Max255 characters"><br/>
