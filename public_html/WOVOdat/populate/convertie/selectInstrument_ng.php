...
if($stationdis == 'check1' && $instrucomponent='noinstru1'){  // Get Network
	...
	if($stationdisplay == "SeismicInstrument" || $stationdisplay=="SeismicComponent"){
		...
		if($stationdisplay == "SeismicInstrument" || $stationdisplay=="SeismicComponent"){  
			...
			if($kilometer != "all"){
				...
				$sql="select s.".$nettype."_name from $nettype as s, cn, vd where s.cn_id = cn.cn_id and cn.vd_id = vd.vd_id and vd.vd_name = '$volca' and cn.cn_type='$stationvalue' and cn.cn_name='$networkdisplay' union select s.".$nettype."_name from $nettype as s, jj_volnet as j, vd, vd_inf as d where s.cn_id = j.jj_net_id and j.vd_id = vd.vd_id and vd.vd_id = d.vd_id and vd.vd_name = '$volca' and j.jj_net_flag='C' and (sqrt(power(d.vd_inf_slat - s.".$latlon."lat, 2) + power(d.vd_inf_slon - s.".$latlon."lon, 2))*100)< '$kilometer'";
		...
		if(isset($data[1])){ //Note: $data[1]
			...
			echo "<br/><span id='pstat'>Station: </span><br/>";
	...
	if($stationdisplay == 'SeismicComponent') {
		...
		if($kilometer != "all"){
			...
			$result = mysql_query("select si.si_name from si,sn,ss,vd where si.ss_id = ss.ss_id and sn.sn_id = ss.sn_id and sn.vd_id = vd.vd_id and vd.vd_name= '$volca' and sn.sn_name= '$networkdisplay' and ss.ss_name = '$stationdis' union select distinct si.si_name FROM jj_volnet as j, ss, si,cc, vd_inf as vf, vd WHERE j.vd_id =vf.vd_id and j.jj_net_id = ss.sn_id and cc.cc_id=ss.cc_id and vd.vd_id=vf.vd_id and ss.ss_id= si.ss_id and vd.vd_name= '$volca' and j.jj_net_flag = 'S' and (sqrt(power(vf.vd_inf_slat - ss.ss_lat, 2) + power(vf.vd_inf_slon - ss.ss_lon, 2))*100)<= '$kilometer'") or die(mysql_error());
