...
//	$result="select distinct ss.ss_name FROM jj_volnet as j, ss, cc, vd_inf as vf, vd WHERE j.vd_id =vf.vd_id and j.jj_net_id = ss.sn_id and cc.cc_id=ss.cc_id and vd.vd_id=vf.vd_id and vd.vd_name= '$volca' and j.jj_net_flag = 'S'";
	
$result="select distinct s.ss_name from ss as s,sn as n,vd  where s.sn_id = n.sn_id and n.vd_id = vd.vd_id and vd.vd_name='$volca' Union select distinct ss.ss_name FROM jj_volnet as j, ss, cc, vd WHERE j.vd_id =vd.vd_id and j.jj_net_id = ss.sn_id and cc.cc_id=ss.cc_id and vd.vd_name= '$volca' and j.jj_net_flag = 'S'";	
	...
	if($stationdisplay == "EDMData"){
		$sql="select s.ds_name from ds as s, cn, vd, di_gen as di where s.cn_id = cn.cn_id and cn.vd_id = vd.vd_id and s.ds_id= di.ds_id and vd.vd_name = '$volca' and cn.cn_type='Deformation' and (di.di_gen_type ='EDM' OR di.di_gen_type ='EDM_Reflector' OR di.di_gen_type ='Total_Station' OR di.di_gen_type ='OtherTypes')";
		$result = mysql_query($sql);	
	}else if($stationdisplay == "AngleData"){
		$sql="select s.ds_name from ds as s, cn, vd, di_gen as di where s.cn_id = cn.cn_id and cn.vd_id = vd.vd_id and s.ds_id= di.ds_id and vd.vd_name = '$volca' and cn.cn_type='Deformation' and (di.di_gen_type ='Angle' OR di.di_gen_type ='Total_Station' OR di.di_gen_type ='OtherTypes')";
		$result = mysql_query($sql);
