...
if($conv == 'PlumeData' || $conv=="plume_satellite_type"){
	...
	$gdplumassindex = array_search('gd_plu_mass',$csvheader);                /*  Nang added on 03-Dec-2014  */ 
	$gdpluventmassindex = array_search('gd_plu_ventmass',$csvheader);        /*  Nang added on 03-Dec-2014  */ 
	$gdplumaxmassindex = array_search('gd_plu_maxmass',$csvheader);          /*  Nang added on 03-Dec-2014  */ 
...
if($conv == "plume_satellite_type"){        
	$imagelink="/".$first2digitvol."/".$second2digitvol."/gas";      // Updated on 18-Dec-2013

	$gd_plu_img_index = array_search('gd_plu_image',$csvheader);     //take index from xml 
...
while(!feof($handle)){
	...
	if($conv == "ThermalImage and ThermalImageData" || $conv == "ThermalImage_satellite_type"){ 
		...
		if($imageName != "") {
			$sortedline[$td_img_path_index] = $imagelink;       // assign image link to XML
		}
	}	
	
	if($conv == "RepresentativeWaveform"){
		
		$imageName = $orgline[$sd_wav_img_index];           // get the image name from CSV
		
		if($imageName != "") {
			$sortedline[$sd_wav_link_index] = $imagelink;       // assign image link to XML
		}
	}
	
	if($conv == "Insar_satellite_type"){                    // InSARImage and InSARData
		
		$imageName = $orgline[$dd_sar_img_index];           // get the image name from CSV
		if($imageName != "") {
			$sortedline[$dd_sar_img_path_index] = $imagelink;   // assign image link to XML
		}
	}
...
/* Added on 17-Dec-2014 --- Only for Gas plume Satellite Data (Ground based station & Airplane data will not have image)*/
	if($conv == "plume_satellite_type"){  
		...
		$imageName = $orgline[$gd_plu_img_index];           // get the image name from CSV
		if($imageName != "") {
			$sortedline[$gd_plu_img_path_index] = $imagelink;   // assign image link to XML
		}
	}
...
/* Nang added on 03-Dec-2014  */		
		if($gdplumassindex){
			$mass = $orgline[$gdplumassindex];	
			if($mass != '' && $mass != 'NULL'){
				$r .="\n\t\t\t<mass>".$mass."</mass>"; 
			}
		}		
		
		if($gdpluventmassindex){
			$ventMass = $orgline[$gdpluventmassindex];	
			if($ventMass != '' && $ventMass != 'NULL'){
				$r .="\n\t\t\t<ventMass>".$ventMass."</ventMass>"; 
			}
		}
		
		
		if($gdplumaxmassindex){
			$maxMass = $orgline[$gdplumaxmassindex];	
			if($maxMass != '' && $maxMass != 'NULL'){
				$r .="\n\t\t\t<maxMass>".$maxMass."</maxMass>"; 
			}
		}
/*Till here Nang added on 03-Dec-2014  */	
