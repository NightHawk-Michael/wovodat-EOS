...
if(isset($_POST['vol2'])){
	...
	if($conv == 'SeismicNetwork' || $conv == 'DeformationNetwork' || $conv == 'GasNetwork' || $conv == 'HydrologicNetwork' || $conv == 'ThermalNetwork' || $conv == 'FieldsNetwork' || $conv == 'MeteoNetwork' ){
		if ($volLength > 1){
			$volcode=getmultivolcode($vol,$volLength);   // Get multiple cavw from DB 
		}else{
			$volcode=getvolcode($vol[0]);  // Get cavw from DB 		
		}	
	}else{
...
if(isset($_POST['satellite'])) {   // Added on 1-june-2012  for gi and ti 
	if($_POST['satellite'] != '' || $_POST['satellite'] != 'NULL'){
...
else if( $conv == 'GasInstrument' || $conv == 'ThermalInstrument'){  // Added on 1-june-2012  
...
//Changed line 271-280 coz of vol list on 18-Mar-2013 	
		
		if($volLength > 1 ){		 
			for($j=0; $j<sizeof($volcode);$j++){
				$r .="\n\t\t\t<volcanoCode>$volcode[$j]</volcanoCode>";
			}
