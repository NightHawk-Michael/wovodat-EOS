...
if($stationdisplay == "ElectronicTiltData" || $stationdisplay == "TiltVectorData" || $stationdisplay == "StrainMeterData" ){
	...
	if($stationdisplay == "EDMData"){
		$sql="select distinct di.di_gen_name from ds as s, cn, vd, di_gen as di where s.cn_id = cn.cn_id and cn.vd_id = vd.vd_id and s.ds_id= di.ds_id and vd.vd_name = '$volca' and cn.cn_type='Deformation' and (di.di_gen_type ='EDM' OR di.di_gen_type ='EDM_Reflector' OR di.di_gen_type ='Total_Station' OR di.di_gen_type ='OtherTypes') and s.ds_name='$stationname'";
		$result = mysql_query($sql);	
	}else if($stationdisplay == "AngleData"){
		$sql="select distinct di.di_gen_name from ds as s, cn, vd, di_gen as di where s.cn_id = cn.cn_id and cn.vd_id = vd.vd_id and s.ds_id= di.ds_id and vd.vd_name = '$volca' and cn.cn_type='Deformation' and (di.di_gen_type ='Angle' OR di.di_gen_type ='Total_Station' OR di.di_gen_type ='OtherTypes' and s.ds_name='$stationname')";
		$result = mysql_query($sql);
		...
		$sql="select distinct di.di_gen_name from ds as s, cn, vd, di_gen as di where s.cn_id = cn.cn_id and cn.vd_id = vd.vd_id and s.ds_id= di.ds_id and vd.vd_name = '$volca' and cn.cn_type='Deformation' and (di.di_gen_type ='Total_Station' OR di.di_gen_type ='OtherTypes' and s.ds_name='$stationname')";
	...
	if(isset($data[2])){   // Note: only show  instrument more than one instrument
		...
		echo"<select name='instrument' id='instrument' style='width:180px;' 57";
